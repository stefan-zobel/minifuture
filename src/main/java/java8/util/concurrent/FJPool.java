 ...
 * @author Doug Lea
 */
final class FJPool extends AbstractExecutorService {
// CVS rev. 1.344
    /*
    ...
    final void runWorker(WorkQueue w) {
        ...
        int r = w.id ^ TLR.nextSecondarySeed();
    ...
    final int awaitJoin(WorkQueue w, FJTask<?> task, long deadline) {
        ...
        if (w != null && task != null) {
            ...
            while (s >= 0) {
                ...
                int r = TLR.nextSecondarySeed() | 1; // odd indices
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) {
        ...
        try {
            ...
            // make the Java 6 compiler happy
            return rethrow(t);
        }
    }

    static <R> R rethrow(Throwable throwable) {
        return FJPool.<R, RuntimeException>erase(throwable);
    }

    @SuppressWarnings("unchecked")
    private static <R, T extends Throwable> R erase(Throwable throwable) throws T {
        throw (T) throwable;
    }
    ...
    static final class MemBar {
        private static final Blank x = new Blank();
        ...
        static {
            try {
                OFF = U.objectFieldOffset(Blank.class.getDeclaredField("v"));
            } catch (Exception e) {
                throw new ExceptionInInitializerError(e);
            }
        }
