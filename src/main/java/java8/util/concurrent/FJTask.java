 ...
 * FJPool#commonPool()} via {@code #fork}, {@link #invoke}, or
 * related methods.  Once started, it will usually in turn start other
 * subtasks.  As indicated by the name of this class, many programs
 * using {@code ForkJoinTask} employ only methods {@code #fork} and
 * {@code #join}, or derivatives such as {@code
 ...
 * coordination mechanisms are {@code #fork}, that arranges
 * asynchronous execution, and {@code #join}, that doesn't proceed
 ...
 * tasks that are never joined (for example, those subclassing {@code
 * CountedCompleter}) often fall into this category.  (2) To minimize
 * resource impact, tasks should be small; ideally performing only the
 * (possibly) blocking action. (3) Unless the {@link
 * FJPool.ManagedBlocker} API is used, or the number of possibly
 * blocked tasks is known to be less than the pool's {@code
 * FJPool#getParallelism} level, the pool cannot guarantee that
 * enough threads will be available to ensure progress or good
 * performance.
 *
 * <p>The primary method for awaiting completion and extracting
 * results of a task is {@code #join}, but there are several variants:
 ...
 * {@code #isCompletedNormally} is true if a task completed without
 * cancellation or encountering an exception; {@link #isCancelled} is
 * true if the task was cancelled (in which case {@link #getException}
 * returns a {@link java.util.concurrent.CancellationException}); and
 * {@code #isCompletedAbnormally} is true if a task was either
 ...
 * for those that do, and {@code java.util.concurrent.CountedCompleter}
 ...
 * {@link java.util.concurrent.Phaser}, {@code #helpQuiesce}, and {@code #complete}) that
 * may be of use in constructing custom subclasses for problems that
 * are not statically structured as DAGs. To support such usages, a
 * ForkJoinTask may be atomically <em>tagged</em> with a {@code short}
 * value using {@code #setForkJoinTaskTag} or {@link
...
// CVS rev. 1.117
    ...
    private int externalInterruptibleAwaitDone() throws InterruptedException {
        ...
        if ((s = status) >= 0 && (s = (FJPool.common.tryExternalUnpush(this) ? doExec() : 0)) >= 0) {
            while ((s = status) >= 0) {
                if (U.compareAndSwapInt(this, STATUS, s, s | SIGNAL)) {
                    synchronized (this) {
                        if (status >= 0)
    ...
    public final V get(long timeout, TimeUnit unit)
        ...
        if ((s = status) >= 0 && nanos > 0L) {
            ...
            else if ((s = (FJPool.common.tryExternalUnpush(this) ? doExec() : 0)) >= 0) {
    ...
    static final class AdaptedRunnable<T> extends FJTask<T>
        ...
        AdaptedRunnable(Runnable runnable, T result) {
            this.runnable = Util.requireNonNull(runnable);
            this.result = result; // OK to set this even before completion
        }
        final T getRawResult() { return result; }
        protected final void setRawResult(T v) { result = v; }
        protected final boolean exec() { runnable.run(); return true; }
    ...
    static final class AdaptedCallable<T> extends FJTask<T>
        ...
        AdaptedCallable(Callable<? extends T> callable) {
            this.callable = Util.requireNonNull(callable);
        }
        final T getRawResult() { return result; }
        protected final void setRawResult(T v) { result = v; }
